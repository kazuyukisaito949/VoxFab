...
void CVX_Sim::EnableFeature(const int VXSFEAT, bool Enabled)
{
    ...
    case VXSFEAT_FDM_MODE: EnableFDMMode(Enabled); break;
	}
}
...
void CVX_Sim::EnableFDMMode(bool Enabled)
{
//	EquilibriumModeEnabled = Enabled;
//	if (EquilibriumModeEnabled){
    if (Enabled){
...
void CVX_Sim::VoxArrayToLocalVXC(){
    ResetSimulation();
    for(int i=0; i<VoxArray.size(); i++){
//            int CurIndex = StoXIndexMap[i];
            Vec3D<> CurPos = VoxArray[i].GetNominalPosition();
...
void CVX_Sim::deleteVoxels(){

//    double Threshold_Stress = 0.0004;

    for (std::vector<CVXS_Voxel>::iterator it = VoxArray.begin(); it != VoxArray.end(); it++){

        if(it->GetMaxBondStrain() > Threshold){

            Vec3D<> CurPos = it->GetNominalPosition();
            int CurIndex = pEnv->pObj->GetIndex(CurPos.x,CurPos.y,CurPos.z);
            pEnv->pObj->Structure[CurIndex] = 0;
        }
    }
...
void CVX_Sim::Optimize(){

    double Threshold_Stress = 0.0004;

    for (std::vector<CVXS_Voxel>::iterator it = VoxArray.begin(); it != VoxArray.end(); it++){

        if(it->GetMaxBondStress() < Threshold_Stress && rand()%40==0){

            Vec3D<> CurPos = it->GetNominalPosition();
            int CurIndex = pEnv->pObj->GetIndex(CurPos.x,CurPos.y,CurPos.z);
            pEnv->pObj->Structure[CurIndex] = 0;

//        }else if(it->GetMaxBondStress() > Threshold_Stress*10 && rand()%6==0){

//            Vec3D<> CurPos = it->GetNominalPosition();
//            int CurIndex = pEnv->pObj->GetIndex(CurPos.x,CurPos.y,CurPos.z);
//            pEnv->pObj->Structure[CurIndex] = 1;
//            CurIndex = pEnv->pObj->GetIndex(CurPos.x+1,CurPos.y,CurPos.z);
//            pEnv->pObj->Structure[CurIndex] = 1;
//            CurIndex = pEnv->pObj->GetIndex(CurPos.x+1,CurPos.y+1,CurPos.z);
//            pEnv->pObj->Structure[CurIndex] = 1;
//            CurIndex = pEnv->pObj->GetIndex(CurPos.x,CurPos.y,CurPos.z+1);
//            pEnv->pObj->Structure[CurIndex] = 1;
        }
    }
}

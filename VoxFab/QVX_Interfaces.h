...
public slots:
	...
	void New(void) {Close(); InitializeMatter(1.0, 10, 10, 10);};
	void Save(int Compression = CP_ZLIB, bool NewLoc = false, QString* pNewFilenameOut = NULL);    
	void SaveZLib(void) {Save(CP_ZLIB);};
    void ExportFAV(int Compression = CP_ZLIB, bool NewLoc = false, QString* pNewFilenameOut = NULL);
    void SaveZLib_FAV(void) {ExportFAV(CP_ZLIB);};
    ...
    bool ImportFAV(QString* pFilenameOut = NULL);
    bool ImportSTL(QString* pFilenameOut);
	void ExportSTL(void);
    void ExportSmoothSTL(void);
	void ExportXYZ(void);
	void ExportKV6(void);
    ...
    void GetMeshInfoString(QString* pString) {std::string tmp; GetMeshInfoStr(&tmp); *pString = QString(tmp.c_str());};

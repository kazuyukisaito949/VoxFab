...
struct GL_Light //An OpenGL light structure
{
	...
	void SetLight(float Scale, Vec3D<>* Offset) {
        ...
        float sP_0 = P[0]*Scale+Offset->x;
        float sP_1 = P[1]*Scale+Offset->y;
        float sP_2 = P[2]*Scale+Offset->z;

		float sP[4] = {sP_0, sP_1, sP_2, P[3]};
		glEnable(Index); 
...
public:
    ...
    void GLSetBG(bool White = false) {if (White) GLSetBG(QColor(255, 255, 255)); else GLSetBG(QColor(100,100,100));};

...
void CVX_MeshUtil::GetSmoothMesh(CVX_Object* pObj){
    CMesh GeneratedSmoothMesh;

    CArray3Df OccupancyArray(pObj->GetVXDim(), pObj->GetVYDim(), pObj->GetVZDim());
    int NumPossibleVox = pObj->GetStArraySize();
    for (int g=0; g<NumPossibleVox; g++){
        if (pObj->Structure.GetData(g)>0) OccupancyArray[g] = 1.0;
    }
    CMarchCube::SingleMaterial(&GeneratedSmoothMesh, &OccupancyArray, 0.5, pObj->GetLatticeDim());
    SetSmoothMesh(pObj, &GeneratedSmoothMesh);
}

void CVX_MeshUtil::SetSmoothMesh(CVX_Object* pObj, CMesh* pMeshIn) //links the simulation to an existing surface mesh so it can be deformed correctly
{
//	if (pSimIn != NULL) pSim = pSimIn;
//	if (pSimViewIn != NULL) pSimView = pSimViewIn;
    if (pMeshIn != NULL) DefMesh = *pMeshIn;
    if (!DefMesh.Exists()) return; //can't import if no mesh...
    vfloat FuzzDist = 1.5; //multiple of Lattice dimension to include voxels (radius)
